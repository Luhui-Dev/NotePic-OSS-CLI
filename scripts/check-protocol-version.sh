...
# PROTOCOL_VERSION shipped in NotePic-OSS-Obsidian's latest GitHub Release.
# See PROTOCOL.md §1 and §6 for what this is enforcing and why.
set -euo pipefail

OTHER_REPO="Luhui-Dev/NotePic-OSS-Obsidian"
OTHER_FILE="src/protocol.ts"
AUTH_HEADER=()
if [ -n "${GITHUB_TOKEN:-}" ]; then
  AUTH_HEADER=(-H "Authorization: Bearer ${GITHUB_TOKEN}")
fi

LOCAL_VERSION=$(grep -m1 'PROTOCOL_VERSION' notepic_oss/__init__.py | sed -E 's/.*PROTOCOL_VERSION *= *"([^"]+)".*/\1/')
if [ -z "$LOCAL_VERSION" ]; then
  echo "::error::Could not parse local PROTOCOL_VERSION from notepic_oss/__init__.py."
  exit 1
fi
